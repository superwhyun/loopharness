...
- 초안을 만든 뒤 **판정 없이 무조건 5회 반복 재작성**한다. 리뷰 통과 여부를 묻지 말고, 매 회차마다 이전 버전 전체를 재검토하고 완전한 수정안으로 다시 작성한다. 관점: 누락 요구사항, 성공 기준 충족, 모듈 경계·의존성, 실행 가능한 AC, 범위와 다음 세션 연속성. 5회차 산출물이 최종 계획이다. 반복을 건너뛰거나 횟수를 줄이지 않는다.
